...
def fmodel_chi(par,grad):
    ...
    if (w[0] < watm[0]) or (w[-1] > watm[-1]):
        # print(f'{nc_cp}, {nk_cp}, {optkind_cp}: w not subset of watm, w goes 
        # from '+str(w[0])+' to '+str(w[-1])+' and watm goes from '+str(watm[0])+'
        # to '+str(watm[-1]))
        return 1e10
    ...
    if mwave is not None:
        ...
        # Verify that stellar wavelength scale is a subset of telluric 
        # wavelength scale.
...
def fmod_conti(par,fitobj):
    ...
    '''
    Same as fmod(), but returns continuum and wavelength scale. For use 
    with plotting and CRmask.py
    '''
    ...
    if np.all(np.diff(w) > 0) == False:
        sys.exit('WAVE ERROR 1 - Hitting negative wavelength ',
                    'solution for some reason - pars: {}'.format(par[6:10]))
        return 1e10
    ...
    if mwave is not None:
        ...
        if (w[0] < wspot[0]) or (w[-1] > wspot[-1]):
            sys.exit('WAVE ERROR 3:  w not subset of wspot, w goes from '
                        + str(w[0]) + ' to ' + str(w[-1]) 
                        + ' and wspot goes from ' + str(wspot[0]) + ' to '
                        + str(wspot[-1]))
            return 1e10
        ...
        if (wspot[0] < watm[0]) or (wspot[-1] > watm[-1]):
            sys.exit('WAVE ERROR 3: wspot not subset of satm, wspot goes from '
                        + str(wspot[0]) + ' to ' + str(wspot[-1]) 
                        + ' and watm goes from ' + str(watm[0]) + ' to ' 
                        + str(watm[-1]))
            return 1e10
