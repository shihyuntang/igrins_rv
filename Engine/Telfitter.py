...
# From Telfit:
def WavelengthErrorFunctionNew(pars, datax,datay, telluricx, 
                                telluricy, maxdiff=0.05):
        """
...
# From Telfit:
def FitWavelengthNew(data_originalx, data_originaly, telluricx, 
                        telluricy, fitorder=3, be_safe=True):
        """
        ...
        output = leastsq(WavelengthErrorFunctionNew, pars, args=args, 
...
def suppress_Fit(fitter, data, c_order):
    model = fitter.Fit(data=data, resolution_fit_mode="SVD", adjust_wave="data", 
                        air_wave=False, continuum_fit_order=c_order)
    return model
...
def telfitter(watm_in, satm_in, a0ucut, inparam, night, order, args, 
                masterbeam, c_order,resolutions, logger):
    '''
    Produce synthetic telluric template from fit to telluric standard observation. 
    How and why it works is detailed in comments throughout the code.
    ...
    # Ideally, we'd fit resolution as well since that varies across the 
    # detector. But in practice the Telfit's resolution fits often diverge 
    # to unphysically high values. Ultimately, we only want accurate estimates 
    # for the chemical compositions, which are unaffected by fixing the resolution 
    # at 45000. The final telluric template we'll be synthesizing from this
    # will be at a set high resolution, anyway, and when we need/have needed 
    # to estimate resolution across the detector, we (have) done so via the 
    # fits of the telluric and stellar templates to the observed A0 and GJ281 
    # spectra.
    ...
    names = ["pressure", "temperature", "angle", "resolution", "wavestart", 
            "waveend", "h2o", "co2", "o3", "n2o", "co", "ch4", "o2", "no", 
            "so2", "no2", "nh3", "hno3"]
    ...
    try:
        if args.debug:
            model = fitter.Fit(data=data, resolution_fit_mode="SVD", 
                                adjust_wave="data", air_wave=False, 
                                continuum_fit_order=c_order)
        else:
    ...
    Note
    ----
    resolution_fit_mode = SVD ought to give faster, more accurate fits for the 
    deep telluric lines we mostly see in K band
    air_wave = False, because data in vacuum wavelengths
    adjust_wave =
        From Telfit comments: "Can be set to either 'data' or 'model'. To wavelength 
        calibrate the
        ...
        As it turns out, the model internal to Telfit is still not very precise in 
        wavelength space, since it relies on the HITRAN
        database. Some lines are accurate to 1 m/s, but some less so.

        Hence, our procedure is as follows:

        1) Fit the A0 spectrum using the Livingston telluric template. Get out a 
        precisely calibrated wavelength solution for the spectrum.
            (This is done in A0Fitter, not Telfitter)
        2) Use that wavelength solution was input for Telfit, and let the 
            wavelength scale of the model vary with respect to it.
        3) Using Telfit's best fit parameters, generate a telluric template 
            at high resolution.
        4) To properly calibrate this template in wavelength space, we fit it to 
            a Telfit'd version of the Livingston telluric template, only allowing
    ...
    if args.plotfigs:
        ...
        axes.plot(10*watm_in, satm_in, color='black', alpha=.8, 
            label='data', lw=0.7)
        axes.plot(10*model.x, model.y*cont1, color='tab:red',  alpha=.8, 
            label='model fit', lw=0.7)
        axes.plot(10*model.x, cont1, color='tab:blue', alpha=.8, 
            label='blaze fit', lw=0.7)
        ...
        axes.set_xlabel(r'Wavelength [$\rm \AA$]', size=6, style='normal' , 
            family='sans-serif' )
        axes.legend(fontsize=5, edgecolor='white')
        axes.set_title(
            'A0Telfit_Order{}_{}_{}.png'.format(order, night, masterbeam),
            size=6, style='normal', family='sans-serif')

        fig.savefig(
            '{}/figs_{}/A0Telfit_Order{}_{}_{}.png'.format(
                inparam.outpath, args.band, order, night, masterbeam),
                format='png', bbox_inches='tight', overwrite=True)


    # Generate telluric template with these parameters but at 
    # higher resolution
    ...
    # Set this to be lower to increase resolution of output template!
    Livingston_minimum_wsep = .035/10
    # This would compute template with IGRINS resolution, sensibly coarser than Livingston 
    IGRINS_minimum_wsep     = .130 

    newwave = np.arange(
        np.min(watm_in)-2.5, 
        np.max(watm_in)+2.5, 
        Livingston_minimum_wsep) # in nm
    ...
    # Call the modeler. On rare occasions, this returns an error as noted on 
    # https://github.com/kgullikson88/Telluric-Fitter/issues/9#issuecomment-123074731
    # The cause of this issue is with the gfortran compiler.
    # If this happens, we deliver NAN arrays, and in later parts of the RV analysis, 
    # A0 fits from the nearest compatible observation will be used.
    ...
    # set very low points to zero so that they don't go to NaN when taken to 
    # an exponent by template power in fmodel_chi
    satmTel[(satmTel < 1e-6)] = 0. 
    ...
    for mol in molsout:
        ...
        if args.debug:
            modelM = fitter2.GenerateModel(
                parfittedDmol, nofit=True, air_wave=False)
        else:
            modelM = suppress_GenerateModel(
                fitter2, parfittedDmol, args)
        ...
        if args.debug:
            ...
            axes.plot(10*watm_in, satm_in, color='black', alpha=.8, 
                label='data', lw=0.7)
            axes.plot(10*model.x, rebin_jv(modelM.x,modelM.y,model.x,False)*cont1, 
                color='tab:red',  alpha=.8, label='model fit', lw=0.7)
            axes.xaxis.set_minor_locator(AutoMinorLocator(5))
            axes.yaxis.set_minor_locator(AutoMinorLocator(2))
            axes.tick_params(axis='both', which='both', labelsize=6, 
                right=True, top=True, direction='in')
            axes.set_ylabel(r'Flux', size=6, style='normal', 
                family='sans-serif' )
            axes.set_xlabel(r'Wavelength [$\rm \AA$]', size=6, style='normal',
                family='sans-serif' )
            axes.legend(fontsize=5, edgecolor='white')
            axes.set_title(
                'A0Telfit_Order{}_{}_{}_{}.png'.format(
                    order, night, masterbeam, mol),
                    size=6, style='normal', family='sans-serif')
            fig.savefig(
                '{}/figs_{}/A0Telfit_Order{}_{}_{}_{}.png'.format(
                    inparam.outpath, args.band, order, night, masterbeam, mol),
                    format='png', bbox_inches='tight', overwrite=True)
