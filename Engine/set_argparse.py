...
def _add_common_arguments_1(parser):
    parser.add_argument(
        "targname", action="store", help="Enter your target name, no space",
        type=str
        )
    parser.add_argument(
        "-HorK", dest="band", action="store", 
...
def _add_common_arguments_2(parser):
    parser.add_argument(
        "-Wr", dest="WRegion", action="store",
    ...
    parser.add_argument(
        '-plot', dest="plotfigs", action="store_true",
        help="If set, will generate and save plots from the program."
        )
    parser.add_argument(
        '-n_use', dest="nights_use", action="store",
...
def _add_common_arguments_3(parser):
    parser.add_argument(
        '-t', dest="template", action="store",
        help="Stellar template. Pick from 'synthetic', 'PHOENIX', or \
            'livingston'. Default = 'synthetic'",
        type=str, default='synthetic'
        )
    parser.add_argument(
        '-temp', dest="temperature", action="store",
        help="The synthetic template temperature used, e.g., 5000",
        type=str, default=''
        )
    parser.add_argument(
        '-logg', dest="logg", action="store",
        help="The synthetic template logg used, e.g., 4.5",
        type=str, default=''
        )
    parser.add_argument(
        '-B', dest="B", action="store",
        help="The synthetic template B used in kG, e.g., 2.5",
        type=str, default='0'
        )
    
    ### Below arguments are still under testing, DO NOT USE IT!!!
    parser.add_argument(
        '-binary', dest="binary", action="store_true",
        help="[NOT READY in 1.5.0 alpha! DO NOT USE!] \
                If set, will fit two stellar components to spectra"
        )
    parser.add_argument(
        '-i2', dest="initvsini2", action="store",
        help="Initial vsini of secondary (float, km/s). If no literature \
                value known, use the value given by Step 2",
        type=str, default=''
        )
    parser.add_argument(
        '-v2', dest="vsinivary2", action="store",
        help="Range of allowed vsini #2 variation during optimization, \
                default = 5.0 km/s. Should be set to 0 for final run.",
        type=str, default='5.0'
        )
    parser.add_argument(
        '-t2', dest="template2", action="store",
        help="Secondary stellar template. Pick from 'synthetic', 'PHOENIX', or \
                'livingston'. Default = 'synthetic'",
            type=str, default='synthetic'
            )
    parser.add_argument(
        '-temp2', dest="temperature2", action="store",
        help="The Secondary synthetic template temperature used, e.g., 5000",
        type=str, default=''
        )
    parser.add_argument(
        '-logg2', dest="logg2", action="store",
        help="The Secondary synthetic template logg used, e.g., 4.5",
        type=str, default=''
        )
    parser.add_argument(
        '-B2', dest="B2", action="store",
        help="The Secondary synthetic template B used in kG, e.g., 2.5",
        type=str, default='0'
        )
    parser.add_argument(
        '-f', dest="fluxratio", action="store",
        help="Flux ratio between primary and secondary. If primary is 8 times\
                brighter than secondary, this would be 0.125 (float).",
        type=str, default=''
        )

    parser.add_argument(
        '-sk_check', dest="skip", action="store_true",
...
def _argparse_step0_s():
    ...
    parser = argparse.ArgumentParser(
        ...
        of use in later steps (i.e., generate the PrepData files). If you do \n
        not have the IGRINS Observation MASTERLOG (IGRINS_RV_MASTERLOG.csv, \n
        only for observation done from 2014 to 2019) or that your target is \n
        not in the MASTERLOG, you must construct your own PrepData files. \n
        See ReadMe for more details. \n
        ''', epilog = _epilog
        )
    parser = _add_common_arguments_1(parser)
...
def _argparse_step1():
    ...
    parser = _add_common_arguments_1(parser)
    parser = _add_common_arguments_2(parser)
    return parser.parse_args()
...
def _argparse_step2():
    ...
    parser = _add_common_arguments_1(parser)
    parser = _add_common_arguments_2(parser)
    parser = _add_common_arguments_3(parser)

    parser.add_argument(
        '-i', dest="initvsini", action="store", help="Initial vsini (float, km/s)",
        type=str, default=''
        )
    parser.add_argument(
        '-v', dest="vsinivary", action="store",
    ...
    parser.add_argument(
        "-l_use", dest="label_use", action="store",
...
def _argparse_step3():
    ...
    parser.add_argument(
        '-gS', dest="guesses_source", action="store",
...
def _argparse_step3_interpteff():
    """Take care of all the argparse stuff."""
    parser = argparse.ArgumentParser(
        prog        = 'IGRINS Spectra Radial Velocity Pipeline - Step 3',
        description = '''
    ...
    parser.add_argument(
        '-dteff', dest="dteff", action="store",
        help="Range of allowed Teff variation during optimization, \
                default = 0 K.",
        type=str, default='0'
        )
    return parser.parse_args()
...
def _argparse_step5():
    ...
    parser.add_argument(
        '-comp', dest="component", action="store",
        help="Crosscorrelate 'primary' stellar residuals or \
                (potentially present) 'secondary' stellar residuals?",
        type=str, default=''
        )
    parser.add_argument(
        '-ABkind', dest="ABkind", action="store",
        help="Use individual As and Bs ('separate') or combine them ('combined')?",
        type=str, default=''
        )
    ...
    parser.add_argument(
        '-skipmod', dest="skipmod", action="store_true",
