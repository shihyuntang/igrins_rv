...
def basicclip_above(l_return,l_ref,nslices):
    ...
    for clic in range(nslices):
        l_refchunk    = l_ref[   int(clic*len(l_ref)/float(nslices)) : int((clic+1)*len(l_ref)/float(nslices))]
        l_returnchunk = l_return[int(clic*len(l_ref)/float(nslices)) : int((clic+1)*len(l_ref)/float(nslices))]
        #xj = np.arange(len(l_refchunk))
        #q = np.polyfit(xj,l_refchunk,2)
        #f = np.poly1d(q)
        #l_returnchunk = l_returnchunk[(l_refchunk/f(xj) < max(l_refchunk/f(xj)))];
        l_returnchunk = l_returnchunk[(l_refchunk < max(l_refchunk))];
