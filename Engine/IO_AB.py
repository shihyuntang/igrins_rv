...
def setup_templates(logger, kind='synthetic', band='K', temperature=5000, logg=4.5):
    if (kind == 'synthetic'):
        ...
        if ((band == 'H') & (temperature == 5000) & (logg==4.5)) or ((band == 'H') & (temperature == 6400) & (logg==4.5)):

            if os.getcwd()[-1]=='v':
                stelldata = Table.read(f'./Engine/syn_template/old_templates/syntheticstellar_{band.lower()}band_T{temperature}_logg{logg}.txt',format='ascii')
            else:
                stelldata = Table.read(f'../Engine/syn_template/old_templates/syntheticstellar_{band.lower()}band_T{temperature}_logg{logg}.txt',format='ascii')
        else:
            if os.getcwd()[-1]=='v':
