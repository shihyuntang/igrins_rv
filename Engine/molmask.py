...
def H2Omasker(inparam,args,order,night,watm,satm,molnames,molwaves,molfluxes):
        ...
        for mol in molnames:
            ...
            if mol =='h2o':#!= dominantmol:
                fluxH2O = np.array(satmI.copy()); waveH2O = np.array(watmI.copy());
            else:
                if go:
                    fluxbox = satmI.copy(); tempwave = watmI.copy()
                    go = False    
                else:
        ...
        fluxother = np.array([np.nanmin(fluxbox[:,ii]) for ii in range(len(fluxbox[0,:]))])
        ...
        for k, g in groupby(enumerate(ind), lambda x:x[0]-x[1]):
            ...
            maskwaves.append([waveH2O[group[0]]-halfsep, waveH2O[group[-1]]+halfsep])
        ...
        if args.debug:            
            ...
            plt.savefig(f'{inparam.outpath}/figs_{args.band}/MaskRegions_{order}_{night[:8]}.png', bbox_inches='tight', format='png', overwrite=True) 
            plt.clf()
            plt.close()
